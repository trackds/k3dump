...
request.get("http://10.28.100.48/KDHRMS/login.aspx", getOpt, (err, res, idxHTML) => {
    if (!err) {
        //console.log(idxHTML);
        let $ = cheerio.load(idxHTML);
        let __EVENTTARGET = $("#__EVENTTARGET").val();
        ...
        ddlDatabases = 2;
        let body = `__EVENTTARGET=${__EVENTTARGET}&__EVENTARGUMENT=${__EVENTARGUMENT}&__VIEWSTATE=${urlSafeEcode(__VIEWSTATE)}&txtWorkstation=${txtWorkstation}&hdLanguageType=${hdLanguageType}&hdLoginCmpNumber=${hdLoginCmpNumber}&hdLoginCmpPwd=${hdLoginCmpPwd}&hdDisplayType=${hdDisplayType}&hdNTUser=${hdNTUser}&hdK3User=${hdK3User}&hdLoginSystem=${hdLoginSystem}&rbLoginMode=${rbLoginMode}&ddlDatabases=${ddlDatabases}&txtUserName=${txtUserName}&UserPwd=${UserPwd}&txtBackEmail=${txtBackEmail}&txtUniqueNum=${txtUniqueNum}`;
        opt.body = body;
        // console.log(opt.body);
        request.post("http://10.28.100.48/KDHRMS/login.aspx",opt, (error, response, body) => {
                if (response.headers['set-cookie']) {
                    /**
                     * @type {[]}
                    */
                ...
                request.get("http://10.28.100.48/KDHRMS/ATSManage/PersonCardRecord.aspx?EM_ID=fa746190-6841-4b03-b4d3-765b6f08e29a", getOpt, (err, res, body) => {
                    if (err) {
                        console.error("get PersonCardRecord error::",err);
                    } else {
                        let $ = cheerio.load(body);
                        let __EVENTTARGET = "dgItemList";
                        let __EVENTARGUMENT = "ChangePageSize%2C"+SIZE;
                        let __VIEWSTATE = urlSafeEcode($("#__VIEWSTATE").val());
                        let dgItemList_PageIndex = urlSafeEcode($("#dgItemList_PageIndex").val());;
                        let dgItemList_PageSize = SIZE;
                        let condition = "";
                        let emp_id = "fa746190-6841-4b03-b4d3-765b6f08e29a";
                        ...
                        opt.headers.Cookie = getOpt.headers.Cookie;
                        opt.body = buf;
                        request.post("http://10.28.100.48/KDHRMS/ATSManage/PersonCardRecord.aspx?EM_ID=fa746190-6841-4b03-b4d3-765b6f08e29a", opt, (err, res, data) => {
                            if (err) {
                                console.error(err);
                                return -1;
                            }
